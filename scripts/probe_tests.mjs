...
{
  const sig = await buildMultiTimeframeSignal('EUR/USD', makeCandleData(FX_SELL), 'FOREX', ENV, { session: FIXED_SESSION, newsBlock: null, edgeFeaturesEnabled: false });
  ...
  const crSig = await buildMultiTimeframeSignal('BTC/USD', makeCandleData({ basePrice: 78000, vol: 60, trend: 18, seed: 11 }), 'CRYPTO', ENV, { session: FIXED_SESSION, newsBlock: null, edgeFeaturesEnabled: false });
  ok('[#10a] CRYPTO signal carries no probe audit', getProbeAudit(crSig) === null);

  // FOREX non-SELL (BUY or NO_TRADE) -> no audit
  const fxBuy = await buildMultiTimeframeSignal('EUR/USD', makeCandleData({ basePrice: 1.08, vol: 0.002, trend: 0.0004, seed: 44 }), 'FOREX', ENV, { session: FIXED_SESSION, newsBlock: null, edgeFeaturesEnabled: false });
