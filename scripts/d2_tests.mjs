...
{
  const sig = await buildMultiTimeframeSignal('BTC/USD', makeCandleData(P), 'CRYPTO', ENV, { session: FIXED_SESSION, newsBlock: null, edgeFeaturesEnabled: false });
  ...
  for (const [name, prof] of [['flat', RANGING],
    ...
    const s = await buildMultiTimeframeSignal('BTC/USD', makeCandleData(prof), 'CRYPTO', ENV, { session: FIXED_SESSION, newsBlock: null, edgeFeaturesEnabled: false });
...
console.log('\n── [#11] Bad-pair block SUSPENDED by default (Phase F) ──');
{
  ...
  const sig = await buildMultiTimeframeSignal('USD/JPY', makeCandleData(RANGING), 'FOREX', ENV, { session: FIXED_SESSION, newsBlock: null, edgeFeaturesEnabled: false });
  ...
  try {
    ...
    const sig2 = await buildMultiTimeframeSignal('USD/JPY', makeCandleData(RANGING), 'FOREX', ENV, { session: FIXED_SESSION, newsBlock: null, edgeFeaturesEnabled: false });
...
console.log('\n── [#12] Non-D2 engine behaviour unchanged ──');
{
  ...
  const sig = await buildMultiTimeframeSignal('BTC/USD', makeCandleData(RANGING), 'CRYPTO', ENV, { session: FIXED_SESSION, newsBlock: null, edgeFeaturesEnabled: false });
