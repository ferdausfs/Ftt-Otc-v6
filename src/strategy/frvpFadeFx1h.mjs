/**
 * Fixed Range Volume Profile (FRVP) fade, FX 1h — STANDALONE strategy module.
 *
 * Step 1 of the higher-timeframe validation of the crypto FRVP+R:R finding
 * (feature/frvp-rr: gross edge +0.05..+0.24R/trade swamped ~30x by costs on
 * 1m entries — the hypothesis here is that a 1h native timeframe produces a
 * proportionally larger R (wider stops), bringing cost-as-fraction-of-R down
 * to something tradeable). SINGLE FIXED 1h profile + SINGLE FIXED 1h trigger
 * — no timeframe mixing, and deliberately NOT adaptive multi-timeframe
 * selection (regime-adaptive switching already failed once in this project,
 * FTT3-R; adaptive selection is a separate, more complex hypothesis that is
 * out of scope here by spec).
 *
 * Imports NOTHING from any prior strategy file. The triple-barrier EXIT is
 * resolved by backtest/tripleBarrier.mjs, reused from the crypto branch
 * UNCHANGED — it is timeframe-agnostic by design (maxHoldMs/msPerBar are
 * call parameters). This file mirrors src/strategy/frvpFade.mjs in structure
 * so the verifier can cross-check both; every numeric difference is listed
 * in the frozen-spec table below.
 *
 * ── TWO STANDING CAVEATS (stated here and in every report, never buried) ───
 *  1. FX VOLUME IS A TICK-COUNT PROXY. Forex is decentralized/OTC — there is
 *     no central venue to measure actual traded size. The volume field used
 *     here (histdata.com free tick feed; hourly candle v = number of quote
 *     updates in that hour) is a tick-count proxy. VAH/VAL/POC are built
 *     from that proxy and MUST NOT be read as if built from genuine traded
 *     volume. (The task's original TwelveData framing had the same class of
 *     caveat; TwelveData was unusable for this test — no locally accessible
 *     key, demo whitelist covers only 2 of the 4 pairs, and its forex
 *     time_series rows carry no volume field at all. Source switch documented
 *     in backtest/fetch_fx1h_data.mjs and the report.)
 *  2. THE DATA IS SHORT. The task scoped this as a short-window test
 *     (~60 days of history was the assumed API cap). We fetch 2026-06 ..
 *     2026-09 (partial) from histdata: ~99 calendar days, of which the first
 *     480 hourly candles are warmup — the evaluable window is ~75 calendar
 *     days (~1,190 1h candles/pair). Expect WIDE confidence intervals; no
 *     result here justifies strong confidence, and per-pair buckets are
 *     flagged (not reported as rates) when they do not clear the minimum-30
 *     rule.
 *
 * ── FROZEN SPEC (all parameters committed before the first backtest run) ────
 *
 * Profile context (recomputed for every evaluated 1h candle):
 *   WINDOW = the trailing 480 closed 1h candles (~20 trading days) whose
 *   CLOSE time is strictly before the evaluated candle's close time. The
 *   evaluated (trigger) candle itself is EXCLUDED from its own profile —
 *   the strictest no-lookahead reading. 480 closed 1h candles must be
 *   available or the bar is NO_TRADE (WARMUP_INSUFFICIENT); no decision is
 *   ever evaluated before that. Weekend/holiday hours are simply absent
 *   candles (never fabricated), so the 480-candle window reaches further
 *   back on the calendar across weekends — by design, candle-count based.
 *   BINS = 50 equal-width bins spanning [min low, max high] of the window.
 *   Volume bucketing = UNIFORM SPREAD (identical to the crypto version): a
 *   candle's tick volume is distributed across the bins its [low, high]
 *   span overlaps, proportional to overlap length (a one-price candle h==l
 *   assigns all volume to its containing bin). POC = the bin with maximum
 *   volume (ties -> lowest-priced bin); POC price = bin midpoint. Value
 *   Area expands outward from the POC bin, at each step adding the
 *   neighboring bin (upper or lower) with MORE volume (exact tie -> expand
 *   UP first), until cumulative volume >= 70% of the window total.
 *   VAH = top EDGE of the accumulated range, VAL = bottom EDGE.
 *
 * Trigger (evaluated on the just-CLOSED 1h candle, strict inequalities):
 ...
 *   NO_TRADE (ZERO_RANGE).
 *
 * Exit (triple-barrier, resolved by backtest/tripleBarrier.mjs UNCHANGED):
 ...
 *   Max hold 240 HOURS (10 days) — longer than the crypto version's 120
 *   minutes, since 1h bars need proportionally more time for the same kind
 *   of move to play out; the hold window is wall-clock time, so a weekend
 *   inside it simply contains no candles (the walk resumes after the gap;
 *   no position is force-closed at the Friday close). TIMEOUT exit at the
 *   last candle closing inside the window (own bucket, own realized R,
 *   excluded from the headline win rate). Data-end before resolution ->
 *   CENSORED (reported separately).
 *
 * ── No-lookahead contract ────────────────────────────────────────────────
 * The decision for 1h index i reads ONLY candles[0..i-1] (the 480 most
 * recent of them form the profile) plus candle i's own OHLC (the candle
 * being judged, which is fully closed at the decision moment). Mutating ANY
 * candle after index i cannot change the decision for i — proven by
 * mutation in scripts/frvp_fx1h_tests.mjs. Mutating candle i changes the
 * trigger judgment but NOT the profile (the trigger candle is excluded
 * from its own profile).
 *
 * Candle shape everywhere: { t, o, h, l, c, v } — t = OPEN time in ms UTC;
 * v = tick count (see caveat #1 above — a proxy, not traded volume).
 */

export const FRVPFX_MS_1H = 3_600_000;

// ── Frozen constants, committed before the first backtest run ───────────────
export const FRVPFX_BINS = 50;            // price bins over the window range
export const FRVPFX_VA_PCT = 0.70;        // value area = 70% of window volume
export const FRVPFX_BUFFER_X = 0.1;       // SL buffer = 0.1 x trigger candle range
export const FRVPFX_MAX_HOLD_HOURS = 240; // time barrier (wall-clock hours = 10 days)
export const FRVPFX_WINDOW_BARS = 480;    // trailing ~20 trading days of 1h candles
export const FRVPFX_TP_R1 = 1.5;          // TP variant (a), in R
export const FRVPFX_TP_R2 = 3.0;          // TP variant (b), in R
...
/**
 * Split candle c's (tick) volume across bins over [lo, lo + width*bins].
 * Returns an array of [binIndex, volume] pairs (1..bins entries, usually
 * 1-3 for 1h candles). Deterministic; contributions sum to c.v (within FP).
 ...
 * the caller; for production that is 480 closed 1h candles strictly before
...
export function buildProfile(candles, opts = {}) {
  const bins = opts.bins ?? FRVPFX_BINS;
  const vaPct = opts.vaPct ?? FRVPFX_VA_PCT;
  ...
  // Value area: expand from POC, richer neighbor first (tie -> UP), until
...
export class FrvpProfileState {
  constructor(opts = {}) {
    this.windowBars = opts.windowBars ?? FRVPFX_WINDOW_BARS;
    this.binsCount = opts.bins ?? FRVPFX_BINS;
    this.vaPct = opts.vaPct ?? FRVPFX_VA_PCT;
 ...
 * path (evaluateFrvpFxBar) and the harness (FrvpProfileState.profile()) feed
 * THIS function, so trigger semantics are identical everywhere.
 */
export function decideFromProfile(p, trig, opts = {}) {
  const bufferX = opts.bufferX ?? FRVPFX_BUFFER_X;
  const tpR1 = opts.tpR1 ?? FRVPFX_TP_R1;
  const tpR2 = opts.tpR2 ?? FRVPFX_TP_R2;
...
/**
 * evaluateFrvpFxBar(windowCandles, trig, opts) — REFERENCE path: builds the
 * profile from the last FRVPFX_WINDOW_BARS candles of windowCandles (the
 * frozen rule: these must be the closed candles strictly before trig's
 * close — the caller slices; slice(-windowBars) enforces the cap) and then
 * applies decideFromProfile. Tests and the verifier use this path on
 * sampled rows.
 */
export function evaluateFrvpFxBar(windowCandles, trig, opts = {}) {
  const windowBars = opts.windowBars ?? FRVPFX_WINDOW_BARS;
